            ...
            <div class="havedProductItem" v-for="(item,index) in product" :key="index">
                ...
                <div class="productDetails">
                    <el-upload class="avatar-uploader" 
                        ...
                        :on-change ="(file,fileList)=>showLocalImage(file,fileList,index)"
...
export default {
        ...
        product:[
            {
                ...
                productLevel:'产品详情介绍',//产品等级
                productNum:2000.00,//产品数量
                readOnly:true,//只读属性
                redact:"编辑",//
                imageUrl: '',//上传产品详情图的图片路径
            },
            {
  ...
  methods:{
      ...
      //本地预览图片， 
      showLocalImage(file,fileList,index){
          let _this = this;
          console.log(file,fileList,index)
        //   _this.product[index].imageUrl = URL.createObjectURL(file.raw);
          _this.product[index].imageUrl = window.URL.createObjectURL(file.raw);
        
      },
      //商品详情图上传前，对图片的格式大小进行检查
...
.manageCommodity>>>.havedProductItem .el-input__inner,.appendProductItem .el-input__inner{   
...
/* 产品类型 */
.manageCommodity>>>.productType input{
    color: rgba(68, 126, 217, 1);
    font-weight: 400;
    font-size: .875rem;
}
/* 产品数量 */
.manageCommodity>>>.productNum input{
    font-weight: 400;
}
/* 产品详情大图上传 */
.manageCommodity>>>.avatar-uploader .el-upload {
...
.manageCommodity>>>.avatar-uploader .el-upload:hover {
    border-color: #409EFF;
  }
.manageCommodity>>>.avatar-uploader-icon {
    font-size: 28px;
    color: #8c939d;
    width: 170px;
    height: 170px;
...
.manageCommodity>>>.avatar {
    width: 170px;
    height: 170px;
    display: block;
  }
/* 编辑按钮 */
.manageCommodity>>>.operateBtn{
    margin-left: 1.25rem
}
/* 添加按钮 */
.manageCommodity>>>.appendBtn{
    width: 80%;
    margin: 0 auto;
    text-align: center;
    
}
.manageCommodity>>>.appendBtn button{
