    ...
    <div class="middle">
        <div class="search">
            ...
            <div :class="{'kind':true,'select':isSelect2}">
                ...
                <ul class="content">
                    <li v-for="(item,index) in productKind" :key="index" @click="chooseKind(item.classification)">{{item.classification}}</li>
                </ul>
            </div>
            <div :class="{'price':true,'select':isSelect3}" >
                <div  @click="chooseTab(3)">价格</div> 
                <div class="selectPrice content">
                    <mt-field  placeholder="最低价" type="number" v-model="minPrice"></mt-field> 
                    <div style="line-height:30px;display:inline-block;margin-top:-10px">-</div>
                    <mt-field  placeholder="最高价" type="number" v-model="maxPrice"></mt-field>
                    <div style="text-align:right;margin-right:9%">
                        <mt-button type="primary" size='small' @click="getDifferentPrice">确认</mt-button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div>
                <table>
                    <tr v-for="(item,index) in rightProduct" :key="index">
                        <td class="firstDiv">
                            <img :src="require('../assets/img/small'+item.product_id+'.jpg')" class="productImg" alt="" @click="toproductDetails(item.product_id)">
                        </td>
                        <td class="secondTd" @click="toproductDetails(item.product_id)">
                            <div class="productInfo product">{{item.product_name}}</div>
                            <div>
                        ...
                        <td>
                            <mt-button type="primary" @click="toproductDetails(item.product_id)">兑换</mt-button>
                        </td>
                    </tr>
                </table>
            </div>
...
export default {
  ...
  methods:{
    ...
    getKind(){
        let _this = this;
        let api = '/classified_navigation_display';
        _this.axios.get(api).then(res=>{
            console.log(res);
            _this.productKind = res.data;
        });
    },
    //选择分类
    chooseKind(classification){
        let _this = this;
        let api = '/classification_show/'+classification;
        _this.axios.get(api).then(res=>{
            console.log(res);
            _this.rightProduct = res.data;
            _this.isSelect2 = false;//让分类tab框隐藏
        });
    },
    //获取不同价格区间的产品
    getDifferentPrice(){
        let _this = this;
        let api = '/price_range/'+_this.minPrice+'/'+_this.maxPrice;
        _this.axios.get(api).then(res=>{
            console.log(res);
            _this.rightProduct = res.data;
            _this.isSelect3 = false;//让价格tab框隐藏
        });
    }
