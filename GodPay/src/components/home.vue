                    ...
                    <tr v-for="(item,index) in rightProduct" :key="index">
                        ...
                        <td>
                            ...
                            <div>
                                ...
                                <img v-if="(index + 1) < 4" :src="require('../assets/img/order'+(index+1)+'.jpg')" alt="" class="levelImg">
                            </div>
                        </td>
                        <td>
                            <mt-button type="primary" @click="toright(item.power_product_grade,item.product_id)">查看详情</mt-button>
                        </td>
                    </tr>
                </table>
            </div>
...
export default {
  ...
  methods:{
    //去认权产品详情页
    toright(sort,product_id){
        let _this = this;
        //console.log('去认权产品');
        if(sort.substr(0,1) == 'v'){//认权产品
            console.log(sort.substr(0,1))
            _this.$router.push({path:'/right/'+sort});
        }else{//认权商品
            _this.$router.push({path:'/productDetails/'+product_id});
        }
